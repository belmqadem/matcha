...
export default function NotificationsPage() {
  ...
  return (
    <div className="relative z-10 flex-1 w-full max-w-3xl mx-auto px-4 py-4">
      {/* Header */}
        ...
        <div>
          ...
          {!loading && (
            <p className="text-sm text-text-muted mt-1">
              {notifications.length === 0 ? 'Nothing here yet' : `${notifications.length} total`}
            </p>
          )}
        </div>
      ...
      {loading ? (
        ...
        <NotificationList notifications={filtered} onRead={markOneAsRead} onDelete={deleteOne} />
      )}
    </div>
  );
}
