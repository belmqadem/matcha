...
import {
  Heart, MapPin, Star, Eye, MessageCircle, Flag, Ban,
...
const ProfilePage = () => {
  ...
  if (error || !profile) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-background">
      <p className="text-[15px] font-medium text-text-muted">{error || 'Profile not found.'}</p>
      <button onClick={() => navigate(-1)} className="text-[14px] font-bold text-primary bg-white px-6 py-2 rounded-full shadow-sm hover:shadow-md transition-all">
        ← Go back
      </button>
    </div>
  );
      ...
      <div className="relative z-10 max-w-[1100px] mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-[1fr_340px] gap-8 items-start">
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row min-h-[340px]">
              ...
              <div className="relative w-full sm:w-[320px] bg-background flex-shrink-0">
                {sorted.length > 0 ? (
                  ...
                  <div className="w-full h-full min-h-[300px] flex items-center justify-center text-text-muted text-6xl font-black bg-background opacity-50">
              ...
              <div className="p-8 flex flex-col justify-between flex-1">
                <div>
                  ...
                  <p className="text-[14px] font-medium text-text-muted mb-4">@{profile.username}</p>

                  {profile.location_city && (
                    <div className="flex items-center gap-1.5 mb-5">
                      <MapPin size={16} className="text-primary" />
                      <span className="text-[15px] font-medium text-text-muted">{profile.location_city}</span>
                    </div>
                  )}
                  ...
                  {profile.biography && (
                    <p className="text-[15px] text-text leading-relaxed mb-6 font-medium opacity-80">
                      "{profile.biography}"
                    </p>
                  )}
                </div>
          ...
          {sorted.length > 1 && (
            <div className="bg-white rounded-[32px] p-7 border border-border shadow-sm animate-[fadeUp_0.5s_ease-out]">
              <h3 className="text-[18px] font-black text-text mb-5">More Photos</h3>
                ...
                {sorted.map((photo, i) => (
                  <div
                    ...
                    className={`relative flex-shrink-0 w-[140px] h-[180px] rounded-2xl overflow-hidden cursor-pointer snap-start transition-all duration-300 ${
                      activePhoto === i ? 'ring-4 ring-primary ring-offset-2' : 'hover:opacity-80'
                    }`}
                  >
          ...
          <div className="bg-white rounded-[32px] p-7 border border-border shadow-sm animate-[fadeUp_0.6s_ease-out]">
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-8">
              {[
                ...
                { label: 'Orientation', value: profile.sexual_preference ? PREFERENCES[profile.sexual_preference] : 'Bisexual' },
              ].map(({ label, value }) => (
                <div key={label} className="border-b-2 border-background pb-3">
                  ...
                  <p className={`text-[15px] font-bold ${value ? 'text-text' : 'text-border'}`}>
                    {value ?? 'Not specified'}
                  </p>
                </div>
              ))}
        ...
        <div className="flex flex-col gap-6 sticky top-8 animate-[fadeUp_0.7s_ease-out]">
          ...
          <div className="bg-white rounded-[32px] p-6 border border-border shadow-sm flex flex-col gap-3">
            ...
            {actionError && (
              <div className="bg-error/10 text-error text-[12px] font-bold px-4 py-3 rounded-xl flex items-center gap-2 mb-2">
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-[18px] font-black text-[15px] transition-all duration-300 active:scale-95 border-2 ${
                profile.is_blocked_by_me
                  ? 'opacity-40 cursor-not-allowed bg-background text-text-muted border-border'
                  : profile.liked_by_me
            ...
            >
              {likeLoading
                ...
                : <Heart size={18} fill={profile.liked_by_me ? 'currentColor' : 'none'} />
              }
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-[18px] font-black text-[15px] transition-all duration-300 ${
                profile.is_connected
                  ? 'bg-text text-white hover:bg-black active:scale-95 shadow-md cursor-pointer'
                  : 'bg-background text-text-muted border-2 border-border opacity-60 cursor-not-allowed'
              }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-[14px] transition-all ${
                profile.is_fake_reported
                  ? 'bg-background text-text-muted cursor-not-allowed border-2 border-border'
                  : 'bg-transparent text-text-muted hover:bg-red-50 hover:text-red-500'
              }`}
            >
              <Flag size={16} />
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between py-2 border-t-2 border-background">
                <div className="flex items-center gap-2 text-[13px] font-bold text-text-muted">
                  <Eye size={16} className="text-primary" /> Profile views
                </div>
                <span className="text-[14px] font-black text-text">Hidden</span>
              </div>
              <div className="flex items-center justify-between py-2 border-t-2 border-background">
                <div className="flex items-center gap-2 text-[13px] font-bold text-text-muted">
                  <Shield size={16} className="text-primary" /> Verified Member
                </div>
          ...
          <button
            onClick={() => navigate(-1)}
            className="w-full py-4 rounded-[18px] border-2 border-border bg-white text-text-muted text-[14px] font-black cursor-pointer hover:border-primary hover:text-primary transition-all shadow-sm active:scale-95"
          >
            ← Back to Browse
          </button>
        </div>
