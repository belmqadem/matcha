      ...
      <div className="flex flex-col gap-5">
        <div className="flex gap-3">
          ...
          <button
            ...
            className="px-6 rounded-xl bg-primary text-white font-bold cursor-pointer hover:shadow-md active:scale-95 transition-all"
          >
            Add
          </button>
        </div>
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <span
                key={tag}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-primary text-white text-[13px] font-bold shadow-sm"
              >
                {tag}
                <button
                  type="button"
                  onClick={() => setTags((t) => t.filter((x) => x !== tag))}
                  className="bg-white/20 rounded-full p-0.5 hover:bg-white/40 transition-colors cursor-pointer"
                >
                  <X size={12} />
                </button>
              </span>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {available.map((tag) => (
                <button
                  ...
                  type="button"
                  onClick={() => addTag(tag)}
                  className="px-3 py-1.5 rounded-full border-2 border-border bg-white text-text-muted text-[13px] font-bold cursor-pointer hover:border-primary hover:text-primary transition-all"
                >
                  {tag}
                </button>
              ))}
