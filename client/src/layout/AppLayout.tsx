...
import {
  ...
  LogOut, Heart, Users, MapPin, Menu, X, Settings,
} from 'lucide-react';
...
const MOBILE_TABS = [
  MAIN_NAV[0], MAIN_NAV[2], MAIN_NAV[3], MAIN_NAV[5], BOTTOM_NAV[0],
];
...
function DotBadge({ count }: { count: number }) {
  ...
  return (
    <span style={{
      ...
      background: 'var(--color-primary)', border: '1.5px solid white',
    }} />
  );
}
...
function Sidebar({
  me, counts, getBadge, handleLogout, loggingOut,
}: {
  me: Me | null;
  counts: NotifCounts;
    ...
    <aside style={{
      ...
      {me && (
        <NavLink to="/profile/me" style={{
          ...
          backgroundColor: bg, textDecoration: 'none',
          ...
          <div style={{
            ...
            border: `3px solid ${primary}`, marginBottom: '10px',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            flexShrink: 0,
          }}>
            {avatar
              ? <img src={avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <span style={{ fontFamily: font, fontSize: '18px', fontWeight: 700, color: primary }}>{initials}</span>
            }
          </div>
          {/* Name */}
          <span style={{
          ...
          {fame && (
            <span style={{ fontFamily: font, fontSize: '11px', color: muted }}>
              Popularity:{' '}
              <span style={{ color: fame.color, fontWeight: 700 }}>{fame.text}</span>
            </span>
          )}
        </NavLink>
      )}
      ...
      {/* Bottom section */}
      <div style={{ borderTop: `1px solid ${border}`, paddingTop: '12px', marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
        <NavLink to="/profile/me" style={{
          display: 'flex', alignItems: 'center', gap: '10px',
          padding: '10px 12px', borderRadius: '12px',
          fontSize: '13px', fontWeight: 500, fontFamily: font,
          textDecoration: 'none', color: isActive('/profile/me') ? primary : muted,
    ...
    <>
      ...
      <header style={{
        ...
        <NavLink to="/profile/me" style={{
          ...
          {avatar
            ? <img src={avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            : <span style={{ fontFamily: font, fontSize: '12px', fontWeight: 700, color: primary }}>{initials}</span>
          }
        </NavLink>
        ...
        {totalUnread > 0 && (
          <span style={{
            ...
            color: primary, fontSize: '11px', fontWeight: 700, fontFamily: font,
          }}>
            {totalUnread}
          </span>
        )}
        ...
        <div ref={menuRef} style={{ position: 'relative' }}>
          <button onClick={() => setMenuOpen(o => !o)} style={{
            ...
            border: 'none', background: menuOpen ? bg : 'none',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          {menuOpen && (
            <div style={{
              ...
              overflow: 'hidden', padding: '8px 0', zIndex: 100,
            }}>
              {[...MAIN_NAV, { to: '/profile/me', label: 'Profile', Icon: User, badge: undefined }].map(({ to, label, Icon, badge }) => {
                const active = isActive(to);
                const count = getBadge(badge as BadgeKey | undefined);
                return (
                  <NavLink key={to} to={to} style={{
                    ...
                    {count > 0 && (
                      <span style={{
                        minWidth: '18px', height: '18px', padding: '0 4px',
                        borderRadius: '999px', background: primary,
                        color: 'white', fontSize: '10px', fontWeight: 700,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                      }}>
                        {count > 99 ? '99+' : count}
              ...
              <button onClick={handleLogout} disabled={loggingOut} style={{
                ...
                cursor: 'pointer', opacity: loggingOut ? 0.5 : 1,
              }}>
                <LogOut size={17} />
